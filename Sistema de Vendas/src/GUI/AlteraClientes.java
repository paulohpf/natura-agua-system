...
public class AlteraClientes extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					AlteraClientes frame = new AlteraClientes();
					frame.setResizable(false);
					frame.setVisible(true);
					frame.setTitle("Consuta de Clientes");
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	public AlteraClientes() throws ParseException {
		...
		clientebd = Banco.retornaClienteBD(id);
		...
		estadoComboBox.setSelectedItem(clientebd[10]);
		...
		cidadeComboBox.setSelectedItem(clientebd[11]);
		...
		nomeField.setText(clientebd[0]);
		formattedTel.setText(clientebd[1]);
		formattedCel.setText(clientebd[2]);
		formattedCPF.setText(clientebd[3]);
		formattedNascimento.setText(clientebd[4]);
		formattedCEP.setText(clientebd[5]);
		enderecoField.setText(clientebd[6]);
		numeroField.setText(clientebd[7]);
		complementoField.setText(clientebd[8]);
		bairroField.setText(clientebd[9]);

		cidades = Banco.retornaCidades(clientebd[10]);
		model2 = new DefaultComboBoxModel(cidades.toArray());
		cidadeComboBox.setModel(model2);
		cidadeComboBox.setEnabled(true);
		cidadeComboBox.setSelectedItem(clientebd[11]);
		...
		btnCadastrar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (cidadeComboBox.getSelectedItem() == null) {
					...
					Banco.alteraCliente(id, nomeField.getText(), formattedTel
							.getText().toString(), formattedCel.getText()
							.toString(), formattedCPF.getText().toString(),
							formattedNascimento.getText().toString(),
							formattedCEP.getText().toString(), enderecoField
									.getText().toString(), numeroField
									.getText().toString(), complementoField
									.getText().toString(), bairroField
									.getText().toString(), cidade, estado);
					AlteraClientes.this.dispose();
				}
			}
		});
